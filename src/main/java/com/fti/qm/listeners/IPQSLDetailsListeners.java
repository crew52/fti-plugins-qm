...
@Service
public class IPQSLDetailsListeners {
    ...
    public final void onQualityCriteriaChange(final ViewDefinitionState view, final ComponentState state, final String[] args) {
        LookupComponent qualityCriteriaField = (LookupComponent) view.getComponentByReference(InProcessQualityStandardLFields.QUALITY_CRITERIA);
        ...
        FieldComponent unitField = (FieldComponent) view.getComponentByReference(InProcessQualityStandardLFields.UNIT);
    ...
    private void calculateUpAndDownValue(final ViewDefinitionState view) {
        FieldComponent quantitativeField = (FieldComponent) view.getComponentByReference(InProcessQualityStandardLFields.QUANTITATIVE_VALUE);
        FieldComponent toleranceField = (FieldComponent) view.getComponentByReference(InProcessQualityStandardLFields.TOLERANCE);
        FieldComponent upValueField = (FieldComponent) view.getComponentByReference(InProcessQualityStandardLFields.UP_VALUE);
        FieldComponent downValueField = (FieldComponent) view.getComponentByReference(InProcessQualityStandardLFields.DOWN_VALUE);
