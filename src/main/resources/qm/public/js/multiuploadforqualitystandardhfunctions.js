$(function() {
    $('#fileupload').fileupload({
        ...
        submit: function(e, data) {
            var locale = window.mainController.getComponentByReferenceName("qSHMultiUploadLocale").getValue().content.value;
            var idField = window.mainController.getComponentByReferenceName("qSHIdForMultiUpload").getValue();
            var idValue = idField.content;

            if (!idValue.value || 0 === idValue.value) {
                $.each(data.files, function(index, file) {
                    showMessage("failure", "QualityStandardH is not saved", "Omitted file upload: " + file.name);
                });
                return false;
            }
        },
